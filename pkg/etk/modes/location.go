...
// LocationCfg is the configuration to start the location history feature.
...
func NewLocation(cfg LocationCfg) (etk.Comp, error) {
	if cfg.Store == nil {
	...
	return etk.WithStates(etk.ComboBox,
		...
		"list/submit", func(it etk.Items, i int) {
			...
			/*
				if err != nil {
					app.Notify(ui.T(err.Error(), ui.FgRed))
				}
				app.PopAddon()
			*/
		},
